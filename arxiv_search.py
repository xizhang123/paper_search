import arxiv
import pandas as pd
from translator import translate_en_zh
...
# 获取文章的发表信息
def get_venue_info(result, abbr_dict, full_dict):
    venue_text = None
    matched_venue = None
    venue_info = ('未收录', '', '')
    
    if result.journal_ref:
        venue_text = result.journal_ref
    elif hasattr(result, 'conference'):
        venue_text = result.conference
    elif result.comment:
        venue_text = result.comment
...
def search_papers(query, max_results=1):
    # 初始化arxiv客户端
    client = arxiv.Client()
    ...
    for result in client.results(search):
        ...
        venue_text, matched_venue, venue_info = get_venue_info(result, abbr_dict, full_dict)
    ...
    df.to_csv("arxiv_papers.csv", index=False, encoding="utf-8-sig", quoting=1)
    
    # 翻译并保存摘要
    translated_abstracts = []
    for abstract_info in abstracts_data:
        abstract = abstract_info['abstract'].replace('\n', ' ')
        abstract = abstract.replace('  ', '').replace('  ', '')
        translated_abstract = translate_en_zh(abstract)
        if translated_abstract:
            translated_abstracts.append({
                **abstract_info,
                "translated_abstract": translated_abstract,
                "abstract_in_line": abstract
            })
    
    # 保存摘要到文件
    with open("arxiv_abstracts.txt", "w", encoding="utf-8") as f_abs:
        for abstract_info in translated_abstracts:
            f_abs.write(f"标题: {abstract_info['title']}\n")
            f_abs.write(f"CCF评级: {abstract_info['ccf_level']}\n")
            f_abs.write(f"发表于: {abstract_info['venue']}\n")
            f_abs.write(f"abstract: {abstract_info['abstract_in_line']}\n")
            f_abs.write(f"摘要：{abstract_info['translated_abstract']}\n\n")
    
    return len(paper_data), len(translated_abstracts)